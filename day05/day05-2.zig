...
pub fn main() !void {
    ...
    std.debug.print("calculating sum of middle page for corrected updates...\n", .{});
    var sum: usize = 0;
    for (updates) |update| {
        // take only initially incorrect updates
        if (getPrintMidlePage(update, page_rules) == null) {
            sum += try correctUpdate(allocator, update, page_rules);
        }
    }
...
fn correctUpdate(allocator: std.mem.Allocator, update: []const u32, page_rules: Rules) !usize {
    // could not get .fromOwnedSlice to work, instead init and then append...
    var update_list = std.ArrayList(u32).init(allocator);
    try update_list.appendSlice(update);
    check_again: while (getPrintMidlePage(update_list.items, page_rules) == null) {

        // find incorrect pages and swap them
        for (update_list.items, 0..) |page, i| {
            // for all page numbers in print order
            if (page_rules.get(page)) |rule| {
                // check rule
                var rule_pages = rule.keyIterator();
                while (rule_pages.next()) |rule_page| {
                    // check if any of the pages in the rules happen before our index
                    if (std.mem.indexOfScalar(u32, update_list.items, rule_page.*)) |rule_page_index| {
                        if (rule_page_index < i) {
                            // swap and continue loop
                            update_list.items[i] = rule_page.*;
                            update_list.items[rule_page_index] = page;
    ...
    return getPrintMidlePage(update_list.items, page_rules).?;
}
...
test correctUpdate {
    ...
    try expectEqual(47, correctUpdate(allocator, updates[3], page_rules));
    try expectEqual(29, correctUpdate(allocator, updates[4], page_rules));
    try expectEqual(47, correctUpdate(allocator, updates[5], page_rules));
}
