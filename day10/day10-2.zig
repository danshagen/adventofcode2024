...
pub fn main() !void {
    ...
    const peaks = try calcTrailheadScores(map);
    std.debug.print("\npeaks: {d}", .{peaks});
}
...
fn calcTrailheadScores(map: [][]u8) !usize {
    var score: usize = 0;
    // for all trailheads
    for (map, 0..) |row, y| {
        for (row, 0..) |height, x| {
            if (height == 0) {
                // trailhead
                score += try findTrails(map, .{ .x = @intCast(x), .y = @intCast(y) });
            }
        }
    }
    return score;
...
test calcTrailheadScores {
    ...
    try expectEqual(81, try calcTrailheadScores(map));
}

fn findTrails(map: [][]u8, start: Position) !usize {
    var score: usize = 0;
    ...
    for (directions) |direction| {
        ...
        if (new_height > height and new_height - height == 1) {
            if (new_height == 9) {
                ...
                // recurse
                score += try findTrails(map, new_position);
            }
        }
    }
    return score;
